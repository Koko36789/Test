...
```mermaid
```
However, the note has to be moved in the same loop, meaning that many loops is needed. Also. the pygame.time.get_ticks() is not reliable in this high frequency of retriving value.
Therefore, I changed the spawning of note to "spawn all before start" to their designated coordinates.

___
At first, i did not set a fps limit. The play loop executes within one miliseconds.The note falling speed Fis exactly 1 miliseond per pixel.
```mermaid
```
...
flowchart TD
    ...
    A3A@{ shape: lin-cyl, label: "Disk storage" }
    B1{run = True}
    B2[[Display menu]]
