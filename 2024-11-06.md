...
> (songname)_score.txt
```
(highest gamplay score of (songname))
```
...
The variable a returnes is a list which contains songname in a[i][0] and contains the composer in a[i][1], where i can \ any integar between 0 and len(a)-1
...
def readlevel(level):
    ...
    return name,composer
```
This program is to retrive information of a specific song to be used in window caption and result screen. Where level is the value indicates the song.
